...
function SectionHeading({ label, accent }: { label: string; accent: string }) {
  return (
    <h2
      className="text-[11px] uppercase tracking-widest font-semibold border-b pb-0.5 mb-2 break-after-avoid"
      style={{ fontFamily: fontFamily('serif'), borderColor: accent, color: accent }}
    >
      {label}
    </h2>
  );
...
export function ClassicTheme({ resume, theme }: Props) {
  ...
  return (
    <div className={`${sp.pad} ${sp.text}`} style={{ fontFamily: fontFamily(theme.bodyFont) }}>
      ...
      <div className="text-center mb-4">
        ...
        {profile.title && (
          <p className="text-[13px] text-gray-600 mt-0.5">{profile.title}</p>
        )}
